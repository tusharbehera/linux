...
static unsigned int rt5625_read_hw_reg(struct snd_soc_codec *codec,
					unsigned int reg)
{
...
#define rt5625_write_mask(c, reg, value, mask) snd_soc_update_bits(c,\
				reg, mask, value)
...
{
	if (rt5625_wait_vodsp_i2c_done(codec))
		return -EBUSY;
						...
						unsigned int vodspreg)
{
	...
	ndata_h = rt5625_read(codec, RT5625_VODSP_REG_DATA);
	...
	ndata_l = rt5625_read(codec, RT5625_VODSP_REG_DATA);
...
static const char *const rt5625_aec_path_sel[] = {
	"aec func disable", "aec func for pcm in/out",
	"aec func for iis in/out", "aec func for analog in/out"
};		/* 0 */
static const char *const rt5625_spk_out_sel[] = {
	"Class AB", "Class D"
};		/* 1 */
static const char *const rt5625_spk_l_source_sel[] = {
	"LPRN", "LPRP", "LPLN", "MM"
};		/* 2 */
static const char *const rt5625_spkmux_source_sel[] = {
	"VMID", "HP Mixer", "SPK Mixer", "Mono Mixer"
};		/* 3 */
static const char *const rt5625_hplmux_source_sel[] = {
	"VMID", "HPL Mixer"
};		/* 4 */
static const char *const rt5625_hprmux_source_sel[] = {
	"VMID", "HPR Mixer"
};		/* 5 */
static const char *const rt5625_auxmux_source_sel[] = {
	"VMID", "HP Mixer", "SPK Mixer", "Mono Mixer"
};		/* 6 */
static const char *const rt5625_spkamp_ratio_sel[] = {
	"2.25 Vdd", "2.00 Vdd", "1.75 Vdd",
	"1.50 Vdd", "1.25 Vdd", "1.00 Vdd"
};		/* 7 */
static const char *const rt5625_mic1_boost_sel[] = {
	"Bypass", "+20db", "+30db", "+40db"
};		/* 8 */
static const char *const rt5625_mic2_boost_sel[] = {
	"Bypass", "+20db", "+30db", "+40db"
};		/* 9 */
static const char *const rt5625_dmic_boost_sel[] = {
	"Bypass", "+6db", "+12db", "+18db",
	"+24db", "+30db", "+36db", "+42db"
};		/* 10 */
static const char *const rt5625_adcr_func_sel[] = {
	...
	unsigned int enable_voice_path, unsigned int mode)
{
	if (enable_voice_path) {
		...
		rt5625_write_mask(codec, RT5625_PWR_MANAG_ADD1, 0,
					PWR_VOICE_DF2SE);
		/* Power down Voice DAC/ADC */
		rt5625_write_mask(codec, RT5625_PWR_MANAG_ADD2, 0,
					PWR_VOICE_CLOCK);
		/* Disable Voice PCM interface */
		rt5625_write_mask(codec, RT5625_EXTEND_SDP_CTRL, 0,
					EXT_I2S_FUNC_ENABLE);
	}
...
static int set_vodsp_aec_path(struct snd_soc_codec *codec, unsigned int mode)
{
	...
	case PCM_IN_PCM_OUT:
		 ...
		 */
		rt5625_write_mask(codec, 0x3a, 0x0c8f, 0x0c8f);
		 ...
		 */
		rt5625_write_mask(codec, 0x3c, 0xa4cb, 0xa4cb);

		/* power spk left/right vol | pwr vodsp interface |
		 * power on microphone1 boost
		 */
		rt5625_write_mask(codec, 0x3e, 0x3302, 0xf302);

		/* Mute DAC to hpmixer */
		rt5625_write(codec, 0x10, 0xee0f);

		/* Set Mic1 to differential mode */
		rt5625_write(codec, 0x0e, 0x8808);

		/* Mic boost 0db */
		rt5625_write(codec, 0x22, 0x0000);

		/* ADC_Mixer_R boost 10.5 db */
		rt5625_write(codec, 0x12, 0xcbd3);

		/* Mic1->ADCMixer_R */
		rt5625_write(codec, 0x14, 0x7f3f);

		/* VoDAC to speakerMixer,0db */
		rt5625_write(codec, 0x18, 0xa010);

		/* Speaker source from speakermixer */
		rt5625_write(codec, 0x1c, 0x8808);
	...
	case ANALOG_IN_ANALOG_OUT:
		...
		/* Set input&output path and power */
		/* Power on related bit */
		rt5625_write_mask(codec, 0x3a, 0xcc8f, 0xcc8f);

		/* Power on related bit */
		rt5625_write_mask(codec, 0x3c, 0xa7cf, 0xa7cf);

		/* Power on related bit */
		rt5625_write_mask(codec, 0x3e, 0xf312, 0xf312);

		/* Set Mic1 to differential mode */
		rt5625_write(codec, 0x0e, 0x8808);

		/* Set phone in to differential mode */
		rt5625_write(codec, 0x08, 0xe800);

		/* Mic boost 0db */
		rt5625_write(codec, 0x22, 0x0000);

		/* Mic1->ADCMixer_R,phone in-->ADCMixer_L */
		rt5625_write(codec, 0x14, 0x773f);

		/* ADC_Mixer_R boost 10.5 db */
		rt5625_write(codec, 0x12, 0xCBD3);

		/* Speaker from spkmixer,monoOut from monoMixer */
		rt5625_write(codec, 0x1c, 0x88c8);

		/* Unmute VoDAC to spkmixer */
		rt5625_write(codec, 0x18, 0xA010);

		/* Unmute DAC to monoMixer */
		rt5625_write(codec, 0x10, 0xee0e);
		...
		/* Unmute auxout */
		rt5625_write_mask(codec, 0x06, 0x0000, 0x8080);
		break;
	...
	case DAC_IN_ADC_OUT:
		...
		/* Set input&output path and power */
		/* Power on related bit */
		rt5625_write_mask(codec, 0x3a, 0xcc0f, 0xcc0f);

		/* Power on related bit */
		rt5625_write_mask(codec, 0x3c, 0xa7cb, 0xa7cb);

		/* Power on related bit */
		rt5625_write_mask(codec, 0x3e, 0x3302, 0x3302);
		...
		rt5625_write(codec, 0x10, 0xee0d);
	...
	default:
		...
		/* Mute auxout */
		rt5625_write_mask(codec, 0x06, 0x8080, 0x8080);

		/* Mic boost 20db by default */
		rt5625_write(codec, 0x22, 0x0500);

		/* Record from Mic1 by default */
		rt5625_write(codec, 0x14, 0x3f3f);

		/* ADC_Mixer_R boost 15 db by default */
		rt5625_write(codec, 0x12, 0xD5D5);

		/* All output from HPmixer by default */
		rt5625_write(codec, 0x1c, 0x0748);

		/* DAC to HPmixer by default */
		rt5625_write(codec, 0x10, 0xee03);

		/* Mute VoDAC to mixer by default */
		rt5625_write(codec, 0x18, 0xe010);
		...
		rt5625_write_mask(codec, 0x3e, 0x0000, 0xf312);

		/* Power down related bit */
		rt5625_write_mask(codec, 0x3a, 0x0000, 0xcc8d);

		/* Power down related bit */
		rt5625_write_mask(codec, 0x3c, 0x0000, 0x07cf);
		break;
	}
	return 0;
}
				...
				unsigned int AEC_mode)
{
	...
	if (enable_vodspAEC != 0) {
		...
		rt5625_write_mask(codec, RT5625_VODSP_CTL, 0,
					VODSP_NO_PD_MODE_ENA);
		/* disable power of VODSP I2C interface & VODSP interface */
		rt5625_write_mask(codec, RT5625_PWR_MANAG_ADD3, 0,
					PWR_VODSP_INTERFACE |
					PWR_I2C_FOR_VODSP);
	}
...
static int rt5625_func_aec_disable(struct snd_soc_codec *codec, int mode)
{
	...
	case RT5625_AEC_ANALOG_IN_OUT:
		...
		rt5625_aec_config(codec, VODSP_AEC_DISABLE);
		break;
	default:
		break;
	}
...
static int rt5625_get_dsp_mode(struct snd_kcontrol *kcontrol,
...
static int rt5625_set_dsp_mode(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	case RT5625_AEC_PCM_IN_OUT:
		...
		rt5625_aec_config(codec, PCM_IN_PCM_OUT);
		break;

	case RT5625_AEC_IIS_IN_OUT:
		/* enable AEC IIS in/out function and path */
		rt5625_aec_config(codec, DAC_IN_ADC_OUT);
		break;

	case RT5625_AEC_ANALOG_IN_OUT:
		/* enable AEC analog in/out function and path */
		rt5625_aec_config(codec, ANALOG_IN_ANALOG_OUT);
		break;

	case RT5625_AEC_DISABLE:
		/* disable previous select function */
		rt5625_func_aec_disable(codec, rt5625_mode);
		break;
...
static const struct snd_kcontrol_new rt5625_snd_controls[] = {
	SOC_ENUM_EXT("rt5625 aec mode sel", rt5625_enum[0],
			rt5625_get_dsp_mode, rt5625_set_dsp_mode),
...
static void hp_mute_unmute_depop(struct snd_soc_codec *codec, int mute)
{
	if (mute) {
		rt5625_write_mask(codec, RT5625_PWR_MANAG_ADD1,
					PWR_SOFTGEN_EN,
					PWR_SOFTGEN_EN);
		rt5625_write(codec, RT5625_MISC_CTRL,
				M_UM_DEPOP_EN | HP_R_M_UM_DEPOP_EN |
				HP_L_M_UM_DEPOP_EN);
		/* Mute headphone right/left channel */
		rt5625_write_mask(codec, RT5625_HP_OUT_VOL,
					RT_L_MUTE|RT_R_MUTE,
					RT_L_MUTE|RT_R_MUTE);
		mdelay(50);
	} else {
		...
		rt5625_write_mask(codec, RT5625_HP_OUT_VOL, 0,
					RT_L_MUTE|RT_R_MUTE);
		mdelay(50);
	}
}
...
static const struct snd_kcontrol_new rt5625_spk_mixer_controls[] = {
	SOC_DAPM_SINGLE("Line Mixer Playback Switch",
		RT5625_LINE_IN_VOL, 14, 1, 1),
	SOC_DAPM_SINGLE("Phone Playback Switch",
		RT5625_PHONEIN_VOL, 14, 1, 1),
	SOC_DAPM_SINGLE("Mic1 Playback Switch",
		RT5625_DAC_AND_MIC_CTRL, 14, 1, 1),
	SOC_DAPM_SINGLE("Mic2 Playback Switch",
		RT5625_DAC_AND_MIC_CTRL, 10, 1, 1),
	SOC_DAPM_SINGLE("DAC Mixer Playback Switch",
		RT5625_DAC_AND_MIC_CTRL, 1, 1, 1),
	SOC_DAPM_SINGLE("Voice DAC Playback Switch",
			...
			struct snd_kcontrol *k, int event)
{
	...
	if ((l & 0x1) || (r & 0x1))
		rt5625_write_mask(codec, RT5625_LINE_IN_VOL,
					0x0000, 0x8000);
	else
		rt5625_write_mask(codec, RT5625_LINE_IN_VOL,
					0x8000, 0x8000);
	...
	if ((l & 0x2) || (r & 0x2))
		rt5625_write_mask(codec, RT5625_PHONEIN_VOL,
					0x0000, 0x8000);
	else
		rt5625_write_mask(codec, RT5625_PHONEIN_VOL,
					0x8000, 0x8000);
	...
	if ((l & 0x10) || (r & 0x10))
		rt5625_write_mask(codec, RT5625_VOICE_DAC_OUT_VOL,
					0x0000, 0x8000);
	else
		rt5625_write_mask(codec, RT5625_VOICE_DAC_OUT_VOL,
					0x8000, 0x8000);
				...
				int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		rt5625_write_mask(codec, RT5625_PWR_MANAG_ADD3,
					0x3000, 0x3000);
		rt5625_write_mask(codec, RT5625_SPK_OUT_VOL,
					0x0000, 0x8080);
		/* power on spk amp */
		rt5625_write_mask(codec, RT5625_PWR_MANAG_ADD1,
					0x0400, 0x0400);
...
static const struct snd_soc_dapm_widget rt5625_dapm_widgets[] = {
	...
	SND_SOC_DAPM_PGA("Mic1 Boost",
				RT5625_PWR_MANAG_ADD3, 1, 0, NULL, 0),
	SND_SOC_DAPM_PGA("Mic2 Boost",
				RT5625_PWR_MANAG_ADD3, 0, 0, NULL, 0),
	...
	SND_SOC_DAPM_MICBIAS("Mic1 Bias",
				RT5625_PWR_MANAG_ADD1, 3, 0),
	SND_SOC_DAPM_MICBIAS("Mic2 Bias",
				RT5625_PWR_MANAG_ADD1, 2, 0),
};
	...
	if (RT5625_PLL1_FROM_MCLK == pll_id) {
		for (i = 0; i < ARRAY_SIZE(codec_master_pll1_div); i++) {
			...
			(freq_out == codec_master_pll1_div[i].pll_out)) {
				...
				/* enable pll1 power */
				rt5625_write_mask(codec,
						RT5625_PWR_MANAG_ADD2,
						0x8000, 0x8000);
				rt5625_write_mask(codec,
						RT5625_GEN_CTRL_REG1,
						0x8000, 0x8000);
				ret = 0;
			}
		}
		...
		int clk_id, unsigned int freq, int dir)
{
	...
	rt5625->stereo_sysclk = DEFAULT_SYSCLK;
		...
		int clk_id, unsigned int freq, int dir)
{
	...
	rt5625->voice_sysclk = DEFAULT_SYSCLK;
					...
					struct snd_soc_dai *dai)
{
	...
	struct rt5625_priv *rt5625 = snd_soc_codec_get_drvdata(codec);
	unsigned int iface = rt5625_read(codec, RT5625_MAIN_SDP_CTRL) &
						0xfff3;
		...
		struct snd_soc_dai *dai)
{
	struct snd_soc_codec *codec = dai->codec;
	struct rt5625_priv *rt5625 = snd_soc_codec_get_drvdata(codec);
	unsigned int iface = rt5625_read(codec, RT5625_EXTEND_SDP_CTRL) &
						0xfff3;
	int rate = params_rate(params);
	...
	/* power i2s and dac ref */
	rt5625_write_mask(codec, 0x3a, 0x0801, 0x0801);
...
static int rt5625_hifi_codec_set_dai_fmt(struct snd_soc_dai *codec_dai,\
						unsigned int fmt)
{
...
static int rt5625_voice_codec_set_dai_fmt(struct snd_soc_dai *codec_dai,\
						unsigned int fmt)
{
	struct snd_soc_codec *codec = codec_dai->codec;
	int iface;
